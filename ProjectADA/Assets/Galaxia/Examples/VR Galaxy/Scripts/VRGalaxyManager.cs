...
public class VRGalaxyManager : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Input.GetKeyDown(KeyCode.V))
		{
			...
			if (adjustCameraRotation) camera.transform.rotation = view.rotation;
		}
	...
	{
		foreach (OffscreenGalaxyRenderer galaxyRenderer in camera.GetComponents<OffscreenGalaxyRenderer>())
		{
			galaxyRenderer.enabled = offscreenParticlesEnabled;
			galaxyRenderer.Hdr = hdr;
		}

		foreach (OffscreenGalaxyRenderer galaxyRenderer in camera.GetComponentsInChildren<OffscreenGalaxyRenderer>())
		{
			galaxyRenderer.enabled = offscreenParticlesEnabled;
			galaxyRenderer.Hdr = hdr;
		}
